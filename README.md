...
### Nice to have, same graph for the prices sensor mentioned above
```
type: custom:apexcharts-card
header:
  show: true
...
series:
  - name: Price
    entity: sensor.tibber_prices_15m
    type: column
    float_precision: 4
    data_generator: >
      const today = entity.attributes.today || []; const tomorrow =
      entity.attributes.tomorrow || []; const all = today.concat(tomorrow); if
      (!all.length) return [];
      ...
      const values = all.map(p => p.total); const min = Math.min(...values);
      const max = Math.max(...values); const range = (max - min) || 1;

      const clamp = (v, lo, hi) => Math.max(lo, Math.min(hi, v)); const toHex =
      (v) => v.toString(16).padStart(2, '0');

      const valueToColor = (v) => {
        const t = clamp((v - min) / range, 0, 1);
        const idx = clamp(Math.round(t * 255), 0, 255);
        const rgb = ironbowPalette[idx] || [255,255,255];
        return `#${toHex(rgb[0])}${toHex(rgb[1])}${toHex(rgb[2])}`;
      };

      return all.map(p => ({
        x: new Date(p.startsAt).getTime(),
        y: p.total,
        fillColor: valueToColor(p.total),
      }));


```
<img width="382" height="266" alt="image" src="https://github.com/user-attachments/assets/23ce2645-77e9-4bee-b72e-52ab0bd8cac0" />
